...
def test_get_answer_grouped_highlights_uses_claims_highlights() -> None:
    chunk_id = uuid.uuid4()
    raw_highlights = [
        {
...
def test_get_answer_grouped_highlights_falls_back_to_claims() -> None:
    answer_row = _make_answer(
        "Ok.",
        raw_citations={"ids": ["one"], "claims": [_raw_claim(Verdict.SUPPORTED)]},
    )
    session = FakeSession()
    session.add(answer_row)
    app.dependency_overrides[get_session] = _override_session(session)
...
def test_get_answer_grouped_highlights_missing_summary_derives() -> None:
    raw_highlights = [
        {
